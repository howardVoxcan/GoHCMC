...
class Graph:
    ...
    def _held_karp(self, start_node, end_node, nodes):
        ...
        if n == 0:
            ...
            return  [start_node, end_node], cost
    ...
    def find_hamiltonian_path(self, fixed_position=None, precedence_constraints=None, start=None, end=None):
        ...
        min_cost = float('inf')
        min_path = None
        free_indices = [i for i, node in enumerate(fixed_position) if node is None]
        free_nodes = [i for i in vertices if i not in set(fixed_position) - {None}]

        for perm_free in permutations(free_nodes):
            candidate = list(perm_free)
            free_idx = 0

            for i in free_indices:
                candidate[i] = perm_free[free_idx]
                free_idx += 1
